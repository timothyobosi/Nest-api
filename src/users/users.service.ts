...
export class UsersService {
  ...
  findAll(role?: 'INTERN' | 'ENGINEER' | 'ADMIN') {
    ...
    if (role) {
      // since its is an arrya we can filter to return users with the role specified
  ...
  findOne(id: number) {
    //finda on method, come in with an id,
  ...
  create(createUserDto: CreateUserDto) {
    ...
    const newUser = {
      ...
      ...createUserDto //spread operator to get the rest of the properties from the user object passed in
  ...
  update(id: number, updatedUserDto: UpdateUserDto) {
    //? means optional for the update
    this.users = this.users.map((user) => {
      // this will spread all the properties of the user
      if (user.id === id) {
        return { ...user, ...updatedUserDto } //the updated user will overwrite the existing user properties
      }
